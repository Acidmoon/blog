...
HUMANIZER_ANTI_AI_STYLE_PROMPT = """Humanizer 去 AI 味检查（基于 blader/humanizer 的博客化改造）：

整体目标：
- 最终文章要像作者本人写出来的博客，而不是 AI 代写稿、新闻通稿、百科条目或公众号模板。
- 允许文字更顺，但不要把棱角、第一人称、真实犹豫、轻微吐槽和个人判断磨平。
- 优先保留原文已有的具体场景、真实动机、成本、踩坑、使用感受和判断；不要用抽象升华替代具体内容。

必须清掉的 AI 味：
- 删除 chatbot 式开场和结尾，例如“当然可以”“下面是”“让我们深入探讨”“希望这对你有帮助”“如果你想了解更多”。
- 避免空泛升华和意义膨胀，例如“具有重要意义”“开启新的篇章”“标志着关键一步”“未来值得期待”“深刻改变行业格局”。
- 避免中文 AI 高频套话，例如“赋能”“打造”“闭环”“生态”“矩阵”“抓手”“高质量内容”“底层逻辑”“商业化路径”。
- 避免假装权威的空话，例如“业内人士认为”“有观点指出”“值得注意的是”“不难发现”，除非原文给了明确来源或上下文。
- 避免机械三段式、排比式、规则三连、强行总结式结构；不要为了整齐牺牲真实表达。
- 避免“不是……而是……”“不仅……更……”“从 X 到 Y”这类模板句，除非原文已有且确实不可替换。
- 避免“首先/其次/最后”“综上所述”“总而言之”等作文腔连接词，除非原文明确就是教程步骤。

博客表达修正：
- 多用直接句、短句和自然转折；少用过长复合句、被动句、空泛形容词。
- 可以重复关键词，不要为了“优雅”机械替换同一个概念，导致读者以为换了对象。
- Markdown 标题要服务阅读，不要每两段就加一个空泛小标题；标题后不要接一句重复标题的废话。
- **加粗**只能用于少量真实关键判断或关键词；不要把每个小节都做成“**标题：** 内容”的 AI 列表。
- 半角 em dash（—）可以保留在标题或少量自然停顿里，但正文不要滥用成 AI 式顿挫。

边界：
- 不新增原文没有的事实、数据、经历、案例、来源、结论。
- 不为了“更有人味”硬加段子、网络梗或情绪。
- 不把技术/研究内容写松：涉及事实、论文、部署步骤和参数时，宁可朴素，也要准确。
"""
...
""" + HUMANIZER_ANTI_AI_STYLE_PROMPT
...
""" + "\n\n" + ACIDMOON_BLOG_STYLE_PROMPT,
        'user_instruction': '请把下面这篇博客草稿整理润色成可直接发布的 Markdown 正文。重点是理解我的原意，尽量贴合原文；补标点、自然分段、适度标画重点，必要时加少量 Markdown 小标题，但不要改写成另一篇文章。润色时必须遵守 Acidmoon / 水浇岭博客长期风格约束，并做 Humanizer 去 AI 味检查。',
    },
    {
        'id': 'khazix_rewrite',
        'label': '卡兹克式公众号改写',
...
""" + "\n\n" + ACIDMOON_BLOG_STYLE_PROMPT,
        'user_instruction': '请把下面这篇博客草稿改写成卡兹克式公众号文章。可以重组节奏、增强表达和主观判断，但不要添加原文没有的新事实、案例或结论。即使使用公众号改写模式，也必须尽量保留 Acidmoon / 水浇岭博客的个人声音和硬性避雷，并做 Humanizer 去 AI 味检查。',
    },
    {
        'id': 'khazix_expand',
        'label': '卡兹克式长文扩写',
...
- 保留 Markdown 语法、链接、代码块、列表的大意。
""" + "\n\n" + ACIDMOON_BLOG_STYLE_PROMPT,
        'user_instruction': '请把下面这份博客素材扩写成一篇更完整的卡兹克式公众号长文。可以扩展原文已有观点和逻辑，但不要添加原文没有的新事实、案例或结论。即使扩写，也必须遵守 Acidmoon / 水浇岭博客长期风格约束，保留作者自己的口吻，并做 Humanizer 去 AI 味检查。',
    },
]
...
- 遵守 Acidmoon / 水浇岭博客长期风格约束和 Humanizer 去 AI 味检查：保留作者自己的口吻、真实动机、具体场景和个人判断；清掉 chatbot 式寒暄、空泛升华、AI 高频套话、机械三段式和“不是……而是……”等模板句。
