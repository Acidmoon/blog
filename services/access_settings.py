...
def _as_bounded_positive_int(value: str, default: int, maximum: int) -> int:
    """Read legacy values safely without allowing an unbounded upload ceiling."""
    return min(_as_positive_int(value, default), maximum)
...
def get_access_settings() -> dict:
    ...
    return {
        ...
        'chat_file_max_mb': _as_bounded_positive_int(
            raw['chat_file_max_mb'],
            10,
            MAX_CHAT_FILE_UPLOAD_MB,
        ),
...
def save_access_settings(form: Mapping[str, str]) -> None:
    ...
    for key, label in fields.items():
        ...
        if key == 'chat_file_max_mb' and value > MAX_CHAT_FILE_UPLOAD_MB:
            raise ValueError(f'{label}不能超过 {MAX_CHAT_FILE_UPLOAD_MB}MB')
        parsed[key] = value
